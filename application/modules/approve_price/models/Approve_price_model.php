...
class Approve_price_model extends CI_Model
{
    ...
    public function getProductDataById($pro_id) {
        $sql = "SELECT
                    t1.product_id,
                    t1.ref_no,
                    t1.group_id,
                ...
                FROM
                    tb_dealer_product AS t1
                    LEFT JOIN ( SELECT * FROM tb_dealer_price_range_product ) AS t2 ON t1.product_id = t2.product_id
                    LEFT JOIN ( SELECT * FROM tb_dealer_price_range ) AS t3 ON t2.price_range_id = t3.price_range_id
                    LEFT JOIN ( SELECT * FROM tb_dealer_suppliers ) AS t4 ON t4.supplier_id = t3.supplier_id
                WHERE
                    1 
                    AND t1.is_delete = '0' 
                    AND t1.product_id = '{$pro_id}' ORDER BY t4.supplier_id
                ";
        $query = $this->db->query($sql);
    ...
    public function getAllPriceRangeById($pro_id) {
        $sql = "SELECT * FROM tb_dealer_price_range WHERE 1 AND supplier_id = '{$pro_id}'";
        $query = $this->db->query($sql);
